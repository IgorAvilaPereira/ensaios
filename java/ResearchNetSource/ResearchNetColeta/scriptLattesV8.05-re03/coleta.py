...
def salvarMatrizXML(cur):
	...
	for i in listaMembros:
		...
		\n<data key="link">http://lattes.cnpq.br/'+i[2]+'</data> \
		\n</node>'
		
	
	cur.execute("select * from adjacencia")
	listaAdjacencia=cur.fetchall()
	
	for i in listaAdjacencia:
		s+='\n<!-- edges --> \
		\n<edge source="'+str(i[0])+'" target="'+str(i[1])+'"> \
		\n<data key="amount">'+str(i[2])+'</data> \
		\n</edge>'
...
if __name__ == "__main__":
	...
	cur.execute("select atual from versao");
	versao = (cur.fetchone()[0]) + 1
	versao = str(versao)
	cur.execute("set search_path to 'versionado'")
	...
	if criarDiretorio(novoGrupo.obterParametro('global-diretorio_de_saida')):
		...
		for i,m in enumerate(novoGrupo.listaDeMembros):
			...
			id_membro=i
			cur.execute("INSERT INTO membros (id_membro, nome,id_lattes, versao) VALUES (%s,%s,%s,%s)",(id_membro,nome,idLattes,versao))			
			for p in m.listaArtigoEmPeriodico:
				tipo="AP"
				...
				cur.execute("INSERT INTO publicacao (tipo, titulo, autores, ano, paginas, id_membro, versao) VALUES (%s,%s,%s,%s,%s,%s,%s) RETURNING id_publicacao", (tipo,titulo,autores,ano,paginas,id_membro,versao))
				...
				cur.execute("INSERT INTO revista (id_publicacao,doi,revista,volume,numero,versao) VALUES (%s,%s,%s,%s,%s,%s)", (id_publicacao,doi,revista,volume,numero,versao))
			...
			for p in m.listaArtigoAceito:
				...
				cur.execute("INSERT INTO revista_aceito (id_publicacao,doi,revista,numero,versao) VALUES (%s,%s,%s,%s,%s)", (id_publicacao,doi,revista,numero,versao))
			...
			for p in m.listaLivroPublicado:
				...
				cur.execute("INSERT INTO livro (id_publicacao,edicao,volume,versao) VALUES (%s,%s,%s,%s)",(id_publicacao,edicao,volume,versao))
			...
			for p in m.listaCapituloDeLivroPublicado:
				...
				cur.execute("INSERT INTO capitulo (id_publicacao,livro,edicao,editora,volume,versao) VALUES (%s,%s,%s,%s,%s,%s)", (id_publicacao,livro,edicao,editora,volume,versao))
			...
			for p in m.listaTrabalhoCompletoEmCongresso:
				...
				cur.execute("INSERT INTO conferencia (id_publicacao,volume,evento,versao) VALUES (%s,%s,%s,%s)",(id_publicacao,volume,evento,versao))
				
				#cur.execute("INSERT INTO autoria (id_membro,id_publicacao) VALUES (%s,%s)",(id_membro, id_publicacao))
				
			for p in m.listaResumoExpandidoEmCongresso:
				tipo="RE"
				autores = p.autores.encode("utf8")
				titulo = p.titulo.encode("utf8").replace("\'","")
				evento = p.nomeDoEvento.encode("utf8")
				...
				cur.execute("INSERT INTO resumo_exp (id_publicacao, doi, evento, volume, versao) VALUES (%s,%s,%s,%s,%s)", (id_publicacao, doi, evento, volume, versao))
			
			for p in m.listaResumoEmCongresso:
				tipo="RC"
				autores = p.autores.encode("utf8")
				titulo = p.titulo.encode("utf8").replace("\'","")
				evento = p.nomeDoEvento.encode("utf8")
				...
				cur.execute("INSERT INTO resumo (id_publicacao, doi, evento, volume, numero, versao) VALUES (%s,%s,%s,%s,%s,%s)", (id_publicacao, doi, evento, volume, numero, versao))
		...
		for rep in novoGrupo.compilador.replicas:
			try:
				cur.execute("INSERT INTO replica (id1,id2,versao) VALUES(%s,%s,%s)",(rep[0],rep[1],versao))
				conn.commit()
		...
		for rep in novoGrupo.compilador.replicas:
			cur.execute("SELECT cluster FROM publicacao WHERE id_publicacao = "+str(rep[0])+" and versao = "+versao)
			rep_pub_1=cur.fetchone()[0]
			if not rep_pub_1:
				cur.execute("UPDATE publicacao SET cluster = "+str(rep[0])+" WHERE id_publicacao="+str(rep[0])+" and versao = "+versao)
				cur.execute("UPDATE publicacao SET cluster = "+str(rep[0])+" WHERE id_publicacao="+str(rep[1])+" and versao = "+versao)
			else:
				cur.execute("UPDATE publicacao SET cluster = "+str(rep_pub_1)+" WHERE id_publicacao="+str(rep[1])+" and versao = "+versao)
		...
		for i,m in enumerate(novoGrupo.listaDeMembros):
			...
			'''
			cur.execute("select id_membro from publicacao where cluster in (select cluster from publicacao where id_membro="+str(i)+" and cluster is not null and versao = "+versao+") and versao = "+versao)
			...
			for c in range(i+1,len(novoGrupo.listaDeMembros)):
				...
				if colaboracao>0:
					cur.execute("INSERT INTO adjacencia (membro1,membro2,colaboracao,versao) VALUES (%s,%s,%s,%s)",(i,c,colaboracao,versao))
		cur.execute("update public.versao set atual = "+versao)
		conn.commit()
	conn.close()
