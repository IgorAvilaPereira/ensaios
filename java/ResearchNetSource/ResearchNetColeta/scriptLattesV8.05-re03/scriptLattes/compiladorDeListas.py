			...
			if listaCompleta.get(pub.ano)==None:   # Se o ano nao existe no listaCompleta (lista total)
				...
				for i in range(0, len(listaCompleta[pub.ano])):
					...
					if not item==None: # sao similares
						...
						if(pub.__class__.__name__ in ("ArtigoAceito","ArtigoEmPeriodico","CapituloDeLivroPublicado","LivroPublicado","TrabalhoCompletoEmCongresso","ResumoEmCongresso","ResumoExpandidoEmCongresso")):
							self.replicas.append(sorted((listaCompleta[pub.ano][i].id_publicacao,pub.id_publicacao)))
