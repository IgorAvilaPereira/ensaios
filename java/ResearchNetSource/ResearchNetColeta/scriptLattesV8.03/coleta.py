...
if __name__ == "__main__":
	...
	if criarDiretorio(novoGrupo.obterParametro('global-diretorio_de_saida')):
		...
		for rep in novoGrupo.compilador.replicas:
			cur.execute("SELECT cluster FROM publicacao WHERE id_publicacao = "+str(rep[0]))
			rep_pub_1=cur.fetchone()[0]
			if not rep_pub_1:
				cur.execute("UPDATE publicacao SET cluster = "+str(rep[0])+" WHERE id_publicacao="+str(rep[0]))
				cur.execute("UPDATE publicacao SET cluster = "+str(rep[0])+" WHERE id_publicacao="+str(rep[1]))
			else:
				cur.execute("UPDATE publicacao SET cluster = "+str(rep_pub_1)+" WHERE id_publicacao="+str(rep[1]))
		...
		for i,m in enumerate(novoGrupo.listaDeMembros):
			...
			'''
			cur.execute("select id_membro from publicacao where cluster in (select cluster from publicacao where id_membro="+str(i)+" and cluster is not null)")
